...
Public Class fCliente
    ...
    Sub buscar(ByVal condicion As String)
        ...
        Dim da As New SqlDataAdapter("SELECT TOP (100) PERCENT ID,Empresa,Cuit,DomicilioFiscal,Mail from Cliente2 where " & condicion & " order by ID", con)
    ...
    Sub CargarCamposCliente()
        ...
        Else
            ...
            Dim da As New SqlDataAdapter("SELECT  upper(ltrim(rtrim(isnull(Empresa,'****')))) as Empresa, upper(ltrim(rtrim(isnull(Alias,'****')))) as Alias,isnull([Cuit],0) as Cuit, ltrim(rtrim(isnull(DomicilioFiscal,''))) as Domicilio_Fiscal,ltrim(rtrim(isnull(Mail,''))) as Mail, ltrim(rtrim(isnull(NumeroDeContacto,''))) as ncontacto,ltrim(rtrim(isnull(Pais,''))) as Pais,isnull([Fax],0) as Fax,upper(ltrim(rtrim(isnull(Usuario,'****')))) as Usuario,upper(ltrim(rtrim(isnull(Clave,'****')))) as Clave,FechaDeAlta as Alta,isnull(estado,0) as Estado from Cliente where ID=" & Val(lLegajo.Text), con)
            Dim ds As New DataSet
            da.Fill(ds, "Cliente")
            TextBox1.Text = ds.Tables("Cliente").Rows(0)("Empresa")
            TextBox2.Text = ds.Tables("Cliente").Rows(0)("Alias")
            TextBox3.Text = ds.Tables("Cliente").Rows(0)("Cuit")
            TextBox4.Text = ds.Tables("Cliente").Rows(0)("Domicilio_Fiscal")
            TextBox5.Text = ds.Tables("Cliente").Rows(0)("Mail")
            TextBox6.Text = ds.Tables("Cliente").Rows(0)("ncontacto")
            TextBox7.Text = ds.Tables("Cliente").Rows(0)("Pais")
            TextBox8.Text = ds.Tables("Cliente").Rows(0)("Fax")
            TextBox9.Text = ds.Tables("Cliente").Rows(0)("Usuario")
            TextBox10.Text = ds.Tables("Cliente").Rows(0)("Clave")
            CheckBox1.Checked = IIf(ds.Tables("Cliente").Rows(0)("estado") = 0, False, True)
            DateTimePicker1.Value = ds.Tables("Cliente").Rows(0)("Alta")
        End If
    End Sub
    ...
    Private Sub bEliminar_Click(sender As Object, e As EventArgs) Handles bEliminar.Click
        If MessageBox.Show("Está por ELIMINAR definitivamente al Cliente: " & TextBox1.Text.Trim.ToUpper & ", " & "Ayudo a que la empresa crezca por años..." & ". Es algo EXTREMO,Sus contribuciones le dieron de comer a miles de familias... Está SEGURO?", "Eliminar Cliente", MessageBoxButtons.YesNo, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.No Then Exit Sub

        If SQL_Accion("delete from ABM.dbo.Cliente  where  ID =" & Val(lLegajo.Text)) = False Then
    ...
    Private Sub bCambio_Click(sender As Object, e As EventArgs) Handles bCambio.Click
        ...
        If SQL_Accion("update ABM.dbo.Cliente set Estado=" & IIf(CheckBox1.Checked, 1, 0) & ", Empresa= '" & TextBox1.Text.Trim.ToUpper.Replace("'", "´") & "', Alias='" & TextBox2.Text.Trim.ToUpper.Replace("'", "´") & "', Cuit='" & TextBox3.Text.Trim.Replace("'", "´") & "', DomicilioFiscal='" & TextBox4.Text.Trim.ToUpper.Replace("'", "´") & "', Mail='" & TextBox5.Text.Trim.ToUpper.Replace("'", "´") & "', NumeroDeContacto='" & TextBox6.Text.Trim.ToUpper.Replace("'", "´") & "', Pais='" & TextBox7.Text.Trim.ToUpper.Replace("'", "´") & "', Fax='" & TextBox8.Text.Trim.ToUpper.Replace("'", "´") & "', Usuario='" & TextBox9.Text.Trim.ToUpper.Replace("'", "´") & "', Clave='" & TextBox10.Text.Trim.ToUpper.Replace("'", "´") & "', FechaDeAlta=" & FechaSQL(DateTimePicker1.Value) & " where ID=" & VNum(lLegajo.Text)) = True Then
    ...
    Private Sub bCrear_Click(sender As Object, e As EventArgs) Handles bCrear.Click
        If SQL_Accion("insert into ABM.dbo.Cliente (Empresa, Alias, Cuit, DomicilioFiscal, Mail, NumeroDeContacto, Pais, FechaDeAlta, Usuario, Clave,estado) values ('**',           '**',           '0',           '***',           '***',           '0',           '***',           getdate(),             '',          '' , 1 )  ") Then
            buscar(" Empresa like '**%' ")
    ...
    Private Sub fCliente_MouseDown(sender As Object, e As MouseEventArgs) Handles MyBase.MouseDown
        m = 1
        mx = e.X
        mi = e.Y
    End Sub
    ...
    Private Sub fCliente_MouseMove(sender As Object, e As MouseEventArgs) Handles MyBase.MouseMove
        If m = 1 Then
    ...
    Private Sub fCliente_MouseUp(sender As Object, e As MouseEventArgs) Handles MyBase.MouseUp
        m = 0
    End Sub
